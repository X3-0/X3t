...
class Mux:
    ...
    def enc(self, sid, data, flags=F_DATA):
        ...
        if len(data) > MAX_PAYLOAD:
            ...
            return r
        return make_frame(sid, data, base_flags, self.c)
    ...
    def dec(self, raw):
        ...
        while off<len(raw):
            ...
            if flags&F_CLOSE:
                ...
                try: 
                    combined = bytes(self.st[sid]) + d
                    res.append((sid,"data",combined))
...
class Server:
    ...
    async def handler(self, websocket):
        cid = id(websocket); mux = Mux(self.dk.a)
    ...
    async def stream(self, cid, sid, typ, d, ws):
        ...
        if sid not in c["socks"] and typ=="data":
            try:
                ...
                port = int(port_str)
                print(f"[*] Connecting {sid} -> {host}:{port}")

                reader, writer = await asyncio.wait_for(
                    asyncio.open_connection(host, port),
                    timeout=10
                )
                c["socks"][sid] = {"reader": reader, "writer": writer, "host": host, "port": port}
                print(f"[+] Stream {sid} connected to {host}:{port}")
        ...
        if sid in c["socks"]:
            try:
                ...
                print(f"[!] Stream {sid} write error: {e}")
                try:
                    await ws.send(make_frame(sid,b"",F_CLOSE,self.dk.a))
                except: pass
                if sid in c["socks"]:
