...
# === Health check — универсально для любой версии websockets ===
...
class Server:
    ...
    async def handler(self, ws, path=None):
        # Универсально: старые версии передают path, новые — нет
        p = path if path is not None else getattr(ws, "path", WS_PATH)
        if p != WS_PATH:
            print(f"[!] Rejected path: {p}")
            await ws.close()
            return
        ...
        print(f"[+] Client {cid} connected from {ws.remote_address}")
        ...
        try:
            ...
            print(f"[-] Client {cid} disconnected: code={e.code}")
        except Exception as e:
            ...
            if client:
                ...
                for sid, (reader, writer) in list(client.get("socks", {}).items()):
                    try:
                        ...
                        pass
            print(f"[-] Client {cid} cleanup done")
    ...
    async def _stream(self, cid, sid, typ, d, ws):
        ...
        if sid not in c["socks"] and typ == "data":
            try:
                ...
                print(f"[*] Stream {sid} connecting to: {dest}")
                if ":" not in dest:
    ...
    async def run(self):
        ...
        # Универсально для любой версии websockets
        try:
            # Пробуем новый API (websockets 14+)
            async with websockets.serve(
                self.handler, "0.0.0.0", PORT, process_request=health_check
            ):
                await asyncio.Future()
        except TypeError:
            # Старый API (websockets < 14)
            print("[*] Using legacy websockets API")
